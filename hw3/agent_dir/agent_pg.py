...
class Agent_PG(Agent):
  ...
  def train(self):
    ...
    with tf.variable_scope('layer_one',reuse=False):
        xavier_l1 = tf.truncated_normal_initializer(mean=0, stddev=1./np.sqrt(n_obs), dtype=tf.float64)
        tf_model['W1'] = tf.get_variable("W1", [n_obs, h], initializer=xavier_l1)
    with tf.variable_scope('layer_two',reuse=False):
        xavier_l2 = tf.truncated_normal_initializer(mean=0, stddev=1./np.sqrt(h), dtype=tf.float64)
    ...
    optimizer = tf.train.RMSPropOptimizer(learning_rate, decay=decay)#############多加min會錯
  ...
  def make_action(self, observation, test=True):
    ...
    x = cur_x - self.prev_x if self.prev_x is not None else np.zeros(n_obs)
    self.prev_x = cur_x

    feed = {tf_x: np.reshape(x, (1,-1))}
    tf_aprob = self.tf_policy_forward(x)
    #aprob = sess.run(tf_aprob,feed) #tf_aprob = tf_aprob[0,:]#跑很慢！
    #action = np.random.choice(n_actions, p=tf_aprob)
    if np.random.uniform() < tf_aprob:
        action = 2
    else: 
        action = 3
    return action# return self.env.get_random_action()
  ...
  def tf_policy_forward(self,x): #x ~ [1,D]
      #h = tf.matmul(self.tf_model['W1'],x)#float32和float64不合無法用
      h = np.dot(self.tf_model['W1'],x)#前後交換！！！
      h[h<0] = 0#h = tf.nn.relu(h)
      #logp = tf.matmul(elf.tf_model['W2'],h)
      logp = np.dot(self.tf_model['W2'],h)
      p = 1.0 / (1.0 + np.exp(-logp))#p = tf.nn.softmax(logp)
      return p
